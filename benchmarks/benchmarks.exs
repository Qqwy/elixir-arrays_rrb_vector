    ...
    Benchee.run(
      ...
      {fn input ->
       ...
       before_each: fn range ->
        ...
        "Arrays.concat/2 (RRBVector)" =>
      {fn input ->
        Arrays.concat(input.lhs, input.rhs)
      end,
       before_each: fn range ->
         %{range: range,
           lhs: range |> Enum.shuffle() |> Arrays.new(implementation: ArraysRRBVector),
           rhs: range |> Enum.shuffle() |> Arrays.new(implementation: ArraysRRBVector)
          }
       end
      },
        "Kernel.++/2 (list)" =>
    ...
    Benchee.run(
      ...
      {fn input ->
       ...
       before_each: fn input ->
        ...
        "Arrays.get/2 (RRBVector)" =>
      {fn input ->
        Arrays.get(input.array, input.index)
      end,
       before_scenario: fn range ->
         %{range: range, array: Arrays.new(range, implementation: ArraysRRBVector)}
       end,
       before_each: fn input ->
         Map.put(input, :index, :rand.uniform(input.range.last) - input.range.first)
       end
    ...
    Benchee.run(
      ...
      {fn input ->
       ...
       before_each: fn input ->
        ...
        "Arrays.append/2 (RRBVector)" =>
      {fn input ->
        Arrays.append(input.collection, input.value)
      end,
       before_each: fn input ->
         Map.put(input, :collection, input.range |> Enum.shuffle |> Arrays.new(implementation: ArraysRRBVector))
       end
      },
        "list ++ [val] (list)" =>
      {fn input ->
